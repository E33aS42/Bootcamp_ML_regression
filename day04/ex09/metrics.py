...
class Metrics():
	...
	def calc_tp_fp_tn_fn(y, y_hat, pos_label=1):
		...
		"""
		if Metrics.check_input(y, y_hat, pos_label):
			tp = 0
			tn = 0
			fp = 0
			fn = 0
	...
	def precision_score_(y, y_hat, pos_label=1):
		...
		"""
		if Metrics.check_input(y, y_hat, pos_label):
			tp, tn, fp, fn = Metrics.calc_tp_fp_tn_fn(y, y_hat, pos_label)
			return tp / (tp + fp)
	...
	def f1_score_(y, y_hat, pos_label=1):
		...
		"""
		if Metrics.check_input(y, y_hat, pos_label):
			r = Metrics.recall_score_(y, y_hat, pos_label)
			p = Metrics.precision_score_(y, y_hat, pos_label)
			return 2 * p * r / (p + r)